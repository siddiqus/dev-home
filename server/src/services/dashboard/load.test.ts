...
describe("computeLoadDistribution", () => {
  ...
  it("computes optional sp and doneSP when issues have storyPoints", () => {
    const issues: EnrichedIssue[] = [
      makeIssue({
        key: "T-1",
        assigneeAccountId: "a1",
      ...
      makeIssue({
        key: "T-4",
        assigneeAccountId: "a1",
        storyPoints: null, // null SP still counts
        statusCategory: "new",
      }),
    ];
    ...
    expect(result[0].sp).toBe(10); // 3 + 5 + 2 (null ignored)
    expect(result[0].doneSP).toBe(7); // 5 + 2
  });

  it("omits sp and doneSP when no issues have storyPoints", () => {
    const issues: EnrichedIssue[] = [
      makeIssue({ key: "T-1", assigneeAccountId: "a1", storyPoints: null }),
      makeIssue({ key: "T-2", assigneeAccountId: "a1", storyPoints: null }),
    ];

    const result = computeLoadDistribution(roster, issues, [], now);

    expect(result[0].sp).toBeUndefined();
    expect(result[0].doneSP).toBeUndefined();
  });

  it("sets sp and doneSP when at least one issue has non-null storyPoints", () => {
    const issues: EnrichedIssue[] = [
      makeIssue({
        key: "T-1",
        assigneeAccountId: "a1",
